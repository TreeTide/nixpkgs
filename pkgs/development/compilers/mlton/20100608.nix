...
stdenv.mkDerivation rec {
  ...
  codeSrc =
    fetchurl {
      ...
      sha256 = "0cqb3k6ld9965hyyfyayi510f205vqzd5qqm3crh13nasvq2rjzj"; # TODO
    };
  ...
  patchPhase = ''
    ...
    sed 's/__gmp_const/const/g' -i `find -type f`
  '';
  ...
  preBuild = ''
    ...
    export LD_LIBRARY_PATH=${gmp4.out}/lib''${LD_LIBRARY_PATH:+:}$LD_LIBRARY_PATH
  ...
  '' + stdenv.lib.optionalString stdenv.isDarwin ''
    ...
    install_name_tool -change /opt/local/lib/libgmp.10.dylib ${gmp4}/lib/libgmp.10.dylib $(pwd)/../${usr_prefix}/lib/mlton/mlton-compile
    install_name_tool -change /opt/local/lib/libgmp.10.dylib ${gmp4}/lib/libgmp.10.dylib $(pwd)/../${usr_prefix}/bin/mlyacc
    install_name_tool -change /opt/local/lib/libgmp.10.dylib ${gmp4}/lib/libgmp.10.dylib $(pwd)/../${usr_prefix}/bin/mllex
  '';
  ...
  postInstall = ''
    ...
    # Path to libgmp.
    substituteInPlace $(pwd)/install/${usr_prefix}/bin/mlton --replace "-link-opt '-lm -lgmp'" "-link-opt '-lm -lgmp -L${gmp4.out}/lib'"

    # Path to gmp.h.
    substituteInPlace $(pwd)/install/${usr_prefix}/bin/mlton --replace "-cc-opt '-O1 -fno-common'" "-cc-opt '-O1 -fno-common -I${gmp4.dev}/include'"
